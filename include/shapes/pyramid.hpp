...
private:
  ...
  bool intersectTriangle(const Ray &r, const vec3 &v0, const vec3 &v1,
                         const vec3 &v2, float &t, vec3 &normal) const {
    vec3 v0v1 = v1 - v0;
    vec3 v0v2 = v2 - v0;
  ...
  bool world_bound(Bounds3f &box) const override {
    float w = width_ / 2.0f;
    box =
        Bounds3f(point3(center_.i_ - w, center_.j_, center_.k_ - w),
                 point3(center_.i_ + w, center_.j_ + height_, center_.k_ + w));
    return true;
  }
};
