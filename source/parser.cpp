...
RGBColor parse_color_format(const std::string& str) {
    ...
    if (v.size() < 3) return RGBColor(0,0,0);
    ...
    if (is_normalized) {
        return RGBColor((unsigned char)(v[0]*255), (unsigned char)(v[1]*255), (unsigned char)(v[2]*255));
    }
    return RGBColor((unsigned char)v[0], (unsigned char)v[1], (unsigned char)v[2]);
}
...
void Parser::parse() const {
    ...
    for (auto it = root->FirstChildElement(); it; it = it->NextSiblingElement()) {
        ...
        for (auto attr = it->FirstAttribute(); attr; attr = attr->Next()) {
            ...
            if (attr_name == "color" || attr_name == "bl" || attr_name == "tl" || attr_name == "tr" || attr_name == "br") {
